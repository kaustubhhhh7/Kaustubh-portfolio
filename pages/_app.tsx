...
const App = ({ Component, pageProps }: AppProps) => {
    return (
    <ReactLenis root options={{ lerp: 0.05, smoothWheel: true, wheelMultiplier: 1.2, orientation: 'vertical', gestureOrientation: 'vertical' }}>
        <Head>
            <meta
            ...
            />
            <meta
                name={'mobile-web-app-orientation'}
                content={'portrait'}
            />
            <script
                dangerouslySetInnerHTML={{
                    __html: `
                        if (typeof window !== 'undefined') {
                            // Block device orientation API to prevent compass indicator
                            if (window.DeviceOrientationEvent) {
                                window.DeviceOrientationEvent.requestPermission = function() { return Promise.reject('denied'); };
                            }
                            if (window.DeviceMotionEvent) {
                                window.DeviceMotionEvent.requestPermission = function() { return Promise.reject('denied'); };
                            }
                        }
                    `
                }}
